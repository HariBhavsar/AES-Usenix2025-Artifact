...
void Rotate(LPBYTE lpbDword) {
	BYTE t;
    t = lpbDword[0];
    for (int i = 0; i < 3; i++)
        lpbDword[i] = lpbDword[i+1];
    lpbDword[3] = t;
}
...
*/
void KeyScheduleCore(LPBYTE lpbDword, int iteration)
{
    Rotate(lpbDword);
	lpbDword[0] = SBoxTLU(lpbDword[0]);
	lpbDword[1] = SBoxTLU(lpbDword[1]);
	lpbDword[2] = SBoxTLU(lpbDword[2]);
	lpbDword[3] = SBoxTLU(lpbDword[3]);

	lpbDword[0] = lpbDword[0]^RconTLU(iteration);
}
...
void KeyExpansion(LPCBYTE key, LPBYTE expandedKey)
{
    ...
    while (currentSize < 176)
    {
        ...
        for(int i = 0; i < 4; i++) {
            expandedKey[currentSize] = expandedKey[currentSize - 16] ^ t[i];
            currentSize++;
        }
    }
}

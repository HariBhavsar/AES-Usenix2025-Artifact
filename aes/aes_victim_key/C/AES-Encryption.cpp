...
void AesEncyption(LPBYTE lpbState, LPCBYTE lpbKey){
	...
	printf("Round 10 sbox outputs: ");
	for(int i = 0; i<16; i++)
		printf("%02x ", lpbState[i]);
	printf("\n");
	ShiftRow(lpbState);
...
void ShiftRow(LPBYTE lpbState){
	BYTE bT;
	bT = lpbState[1]; lpbState[1] = lpbState[5]; lpbState[5] = lpbState[9]; lpbState[9] = lpbState[13]; lpbState[13] = bT;
	bT = lpbState[2]; lpbState[2] = lpbState[10]; lpbState[10] = bT; bT = lpbState[6]; lpbState[6] = lpbState[14]; lpbState[14] = bT;
	bT = lpbState[3]; lpbState[3] = lpbState[15]; lpbState[15] = lpbState[11]; lpbState[11] = lpbState[7]; lpbState[7] = bT;
}

// add the subkey to the state
void KeyAdd(LPBYTE lpbState, LPCBYTE lpbKey, int iRount){
	int j = 0;
	for(int i = iRount * 16; i < (iRount*16+16); i++){
		lpbState[j] ^= lpbKey[i];
		j++;
	}
}
